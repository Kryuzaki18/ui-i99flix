...
function PlayerHeader() {
  ...
  return (
    ...
    >
      ...
      <Link
        to="/"
        className="player-page__header-logo-link"
        style={{ flexShrink: 0 }}
      >
        <img
...
export default function Player() {
  ...
  const { data: movieTmdbDetail } = useTmdbMovieDetailQuery(
    !isTv ? safeId : null,
  );
  ...
  const country = isTv
    ? (tvDetail?.production_countries?.[0]?.name ??
      tvDetail?.origin_country?.[0] ??
      null)
  ...
  const totalEpisodesForSeason =
    tvDetail?.seasons?.find((s) => s.season_number === (manualSeason ?? 1))
      ?.episode_count ?? 30;
  ...
  const watchedEpisodes = useMemo(() => {
    ...
    return new Set(
      watchEntry.episodes.map((ep) => `${ep.season}-${ep.episode}`),
    );
  }, [watchEntry]);
  ...
  const lastWatched = useMemo(() => {
    if (
      !isTv ||
      (hasUrlSeason && hasUrlEpisode) ||
      !watchEntry?.episodes?.length
    )
      return null;
